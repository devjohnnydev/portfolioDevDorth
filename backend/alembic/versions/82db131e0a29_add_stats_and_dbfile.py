"""Add stats and DbFile

Revision ID: 82db131e0a29
Revises: 
Create Date: 2026-04-08 00:26:08.089826
...
def upgrade() -> None:
    ...
    if 'profiles' not in tables:
        ...
        # Check if 'stats' column is missing from profiles
        columns = [c['name'] for c in inspector.get_columns('profiles')]
        if 'stats' not in columns:
            op.add_column('profiles', sa.Column('stats', sa.JSON(), nullable=True))
    ...
    if 'projects' not in tables:
        ...
        sa.Column('tech', sa.String(), nullable=True),
